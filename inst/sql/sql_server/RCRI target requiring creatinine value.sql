...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45887934,45889157,45890368,2006245,4219032,4233085,2002989,4198190,45888248,4078547,45890558,2004322,4117038,2722669,45890135,4242997,2003222,40310989,2005436,2104278,2109123,4281936,2005534,4213313,4321596,4001541,45888842,45889928,45889154,45889709,2000076,2000074,2000073,44821552,4127886,2000091,37396360,4306206,2110647,2002984,2002908,2003223,2004470,2000833,2004451,2004464,2110299,2004467,4298948,4149381,4306298,45888144,2002985,4286744,4238865,4274961,2005478,2005479,2004462,2004468,2002990,2004326,2002971,2003003,2005902,4297365,4181781,4073831,4076035,4162099,2774090,2774059,2753402,2753403,2004306,4200964,4208113,2004450,2005891,4203771,4318111,4053151,4140228)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (45887934,45889157,45890368,2006245,4219032,4233085,2002989,4198190,45888248,4078547,45890558,2004322,4117038,2722669,45890135,4242997,2003222,40310989,2005436,2104278,2109123,4281936,2005534,4213313,4321596,4001541,45888842,45889928,45889154,45889709,2000076,2000074,2000073,44821552,4127886,2000091,37396360,4306206,2110647,2002984,2002908,2003223,2004470,2000833,2004451,2004464,2110299,2004467,4298948,4149381,4306298,45888144,2002985,4286744,4238865,4274961,2005478,2005479,2004462,2004468,2002990,2004326,2002971,2003003,2005902,4297365,4181781,4073831,4076035,4162099,2774090,2774059,2753402,2753403,2004306,4200964,4208113,2004450,2005891,4203771,4318111,4053151,4140228)
  and c.invalid_reason is null
...
WHERE (C.procedure_date >= DATEFROMPARTS(2017, 1, 1) and C.procedure_date <= DATEFROMPARTS(2020, 10, 1))
...
WHERE (C.observation_date >= DATEFROMPARTS(2017, 1, 1) and C.observation_date <= DATEFROMPARTS(2020, 10, 1))
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,0,P.START_DATE) AND A.END_DATE >= DATEADD(day,2,P.START_DATE) AND A.END_DATE <= P.OP_END_DATE
...
WHERE (C.value_as_number >= 0.1000 and C.value_as_number <= 10.0000)
...
WHERE (C.value_as_number >= 20.0000 and C.value_as_number <= 300.0000)
